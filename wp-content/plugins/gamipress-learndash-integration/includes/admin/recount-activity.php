...
function gamipress_ld_activity_recount_quizzes( $response, $loop, $limit, $offset ) {
    ...
    if( $loop === 0 && $users_count > $limit ) {
        ...
        $response['message'] = sprintf( __( '%d users found, recounting...', 'gamipress-learndash-integration' ), $users_count );
        $response['log'] = sprintf( __( '%d users found, recounting...', 'gamipress-learndash-integration' ), $users_count ) . "<br>\n";
    ...
    foreach( $users as $user ) {
        ...
        foreach( $quizzes as $quiz ) {
            ...
            if( $course_id !== 0 ) {
                $response['log'] .= sprintf( __( '[Complete quiz] Quiz: %s User: %s Course: %s Quiz Score: %s', 'gamipress-learndash-integration' ),
                ...
                $response['log'] .= sprintf( __( '[Complete quiz] Quiz: %s User: %s Quiz Score: %s', 'gamipress-learndash-integration' ),
                    '<a href="' . get_edit_post_link( $quiz_id ) . '" target="_blank">' . $quiz_id . '</a>',
                    '<a href="' . get_edit_user_link( $user->ID ) . '" target="_blank">' . $user->ID . '</a>',
                    absint( $quiz['percentage'] )
                ) . "<br>\n";
    ...
    if( $recounted_users < $users_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining users to finish recount', 'gamipress-learndash-integration' ), ( $users_count - $recounted_users ) );
    }
            ...
            foreach( $course['topics'] as $lesson_id => $topics ) {
                ...
                foreach( $topics as $topic_id => $completed ) {
                    ...
                    if( $completed ) {
                        ...
                        $response['log'] .= sprintf( __( '[Complete topic] Topic: %s User: %s Lesson: %s Course: %s', 'gamipress-learndash-integration' ),
        ...
        foreach( $courses as $course_id => $course ) {
            ...
            foreach( $course['lessons'] as $lesson_id => $completed ) {
                ...
                if( $completed ) {
                    ...
                    $response['log'] .= sprintf( __( '[Complete lesson] Lesson: %s User: %s Course: %s', 'gamipress-learndash-integration' ),
                        '<a href="' . get_edit_post_link( $lesson_id ) . '" target="_blank">' . $lesson_id . '</a>',
                        '<a href="' . get_edit_user_link( $user->ID ) . '" target="_blank">' . $user->ID . '</a>',
                        '<a href="' . get_edit_post_link( $course_id ) . '" target="_blank">' . $course_id . '</a>'
                    ) . "<br>\n";
    ...
    foreach( $users as $user ) {
        ...
        foreach( $courses as $course_id => $course ) {
            ...
            if( $completed ) {
                ...
                $response['log'] .= sprintf( __( '[Complete course] Course: %s User: %s', 'gamipress-learndash-integration' ),
                    '<a href="' . get_edit_post_link( $course_id ) . '" target="_blank">' . $course_id . '</a>',
                    '<a href="' . get_edit_user_link( $user->ID ) . '" target="_blank">' . $user->ID . '</a>'
                ) . "<br>\n";
            }
