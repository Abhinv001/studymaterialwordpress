...
class MyCred_Learndash_Leaderboard {
    ...
    public function __construct() {
        ...
        add_action('admin_enqueue_scripts', array($this, 'mycred_learndash_leaderboard_scripts'));
    ...
    public function mycred_learndash_course_options() {
        ...
        $select_all = $course_based_meta['all'];
    ...
    function mycred_learndash_select_a_lesson_ajax() {
        ...
        $learndash_lesson = $lesson_meta['learndash_lesson'];
        if ($post_array) {
            ?>
    ...
    function mycred_learndash_select_a_quiz_ajax() {
        ...
        $learndash_topic_quiz = $lesson_topic_meta['learndash_lesson_topic'];
        if ($lessons_topics) {
            ?>
    ...
    public function mycred_leaderboard_based_topic() {
        ...
        $based_topic = $topic_based['leaderboard_based_topic'] ? $topic_based['leaderboard_based_topic'] : '';
    ...
    public function mycred_learndash_show_based_quiz() {
        ...
        $based_quiz = $quiz_based['leaderboard_based_quiz'] ? $quiz_based['leaderboard_based_quiz'] : '';
    ...
    public function mycred_learndash_leaderboard_save( $post_id, $post, $update ) {
        ...
        $mycred_leaderboard_box_nonce = isset($_POST['mycred_leaderboard_box_nonce']) ? absint($_POST['mycred_leaderboard_box_nonce']) : 0;

        if ( !wp_verify_nonce( $mycred_leaderboard_box_nonce, basename(__FILE__) ) ) return $post_id;

        if ( isset( $_POST['leaderboard_setting'] ) ) {
            
            $leaderboard_setting = array();

            $leaderboard_setting_values = isset($_POST['leaderboard_setting']) ? absint($_POST['leaderboard_setting']) : 0;
            
            foreach ( $leaderboard_setting_values as $key => $value ) {
                if ( 
                    ( ! @in_array( $key , 'leaderboard_pagination', 'leaderboard_timefilter' ) ) ||
        ...
        if ( isset( $_POST['leaderboard_course_based'] ) ) {
            ...
            $leaderboard_course_based_values = isset($_POST['leaderboard_course_based']) ? absint($_POST['leaderboard_course_based']) : 0;
        ...
        if ( isset( $_POST['leaderboard_topic_based'] ) ) {
            ...
            $leaderboard_topic_based_values = isset($_POST['leaderboard_topic_based']) ? absint($_POST['leaderboard_topic_based']) : 0;
        ...
        if ( isset( $_POST['leaderboard_quiz_based'] ) ) {
            ...
            $leaderboard_quiz_based_values = isset($_POST['leaderboard_quiz_based']) ? absint($_POST['leaderboard_quiz_based']) : 0;
        ...
        if ( isset( $_POST['leaderboard_lesson_based'] ) ) {
            ...
            $leaderboard_lesson_based_values = isset($_POST['leaderboard_lesson_based']) ? absint($_POST['leaderboard_lesson_based']) : 0;
    ...
    function mycred_leaderboard_generate_shortcode( $post_id ) {
        ...
        $settings = get_post_meta($post_id, 'leaderboard_default', true);
                    ...
                    <div class="form-element">
                        <div class="which-point">
                            <div>
                                <?php
                                if (mycred_get_types()) {
                                    $pt_meta = $default['leaderboard_pt_type'];
                                    foreach (mycred_get_types() as $key => $pt_type) {
                                        ?>
                                        <div class="points-wrapper">
                                            <div class="type-point">
                                                <?php
                                                if (is_array($pt_meta) && in_array($key, $pt_meta)) {
                                                    $checked = 'checked="checked"';
                                                } else {
                                                    $checked = null;
                                                }
                                                ...
                                                <input type="checkbox" name="leaderboard_setting[leaderboard_pt_type][]"
                                                       value="<?php echo esc_attr($key); ?>" <?php echo (is_array($pt_meta) && in_array($key, $pt_meta)) ? 'checked="checked"' : '' ?> />
                            ...
                            <div class="timing-wrapper">
                                ...
                                <div id="custom-picker">
                                    <label for="from">From</label> <input type="text" id="from"
                                                                          ...
                                                                          value="<?php echo esc_attr($custom_date[0]); ?>" />
                                    <label for="to">to</label> <input type="text" id="to"
                                                                      name="leaderboard_setting[mycred_leaderboard_custom_date][]"
                                                                      value="<?php echo esc_attr($custom_date[1]); ?>" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-element">
                            <div class="user-count">
                                <input type="number" name="leaderboard_setting[leaderboard_users]"
                                       value="<?php echo esc_attr($default['leaderboard_users']) ? esc_attr($default['leaderboard_users']) : 10; ?>" /><?php esc_html_e('Users', 'mycred-learndash'); ?>
                            </div>
                        </div>
                    ...
                    <div class="form-element">
                        <div class="show-pag">
                            <input type="checkbox" value="1" id="show-pagination" name="leaderboard_setting[leaderboard_pagination]"
                                   <?php echo $default['leaderboard_pagination'] ? 'checked' : '' ?> /><?php esc_html_e('Yes', 'mycred-learndash'); ?>
                            <div id="pagination-number">
                                <input type="number" name="leaderboard_setting[leaderboard_pagination_value]"
                                       value="<?php echo esc_attr($default['leaderboard_pagination_value']); ?>" /><?php esc_html_e('Users per page', ',mycred-learndash'); ?>
                            </div>
                        </div>
                    </div>
